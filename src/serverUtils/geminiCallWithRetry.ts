  ...
  for (const modelName of candidateModels) {
    for (let attempt = 1; attempt <= maxAttemptsPerModel; attempt++) {
      try {
        ...
        // If high demand 503 error, immediately fall through to next candidate model
        ...
        if (isHighDemand) {
          console.info(`${logPrefix} Model ${modelName} experiencing high demand; falling over immediately to next model.`);
          break;
        }
